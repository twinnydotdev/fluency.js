  ...
  const convertedTools: MessageCreateParamsNonStreaming['tools'] = tools.map(
    (tool) => {
      return {
        ...
        ...(opts?.anthropic?.caching?.tools === true && {
          cache_control: {
            type: 'ephemeral',
          },
        }),
  ...
  for (const message of clonedMessages) {
    ...
    if (message.role === 'tool') {
      const toolResult: ToolResultBlockParam = {
        ...
        content: message.content,
        type: 'tool_result',
        ...(opts?.anthropic?.caching?.toolUse === true && {
          cache_control: {
            type: 'ephemeral',
          },
        }),
      ...
      if (typeof message.content === 'string') {
        currentParams.push({
          ...
          type: 'text',
          ...(opts?.anthropic?.caching?.messages === true && {
            cache_control: {
              type: 'ephemeral',
            },
          }),
          ...
          message.content.map(async (e) => {
            if (e.type === 'text') {
              ...
              return {
                ...
                ...(opts?.anthropic?.caching?.images === true && {
                  cache_control: {
                    type: 'ephemeral',
                  },
                }),
    ...
    systemMessageWithCache = [
      {
        type: 'text',
        text: systemMessage,
        ...(opts?.anthropic?.caching?.systemMessage === true && {
          cache_control: {
            type: 'ephemeral',
          },
        }),
